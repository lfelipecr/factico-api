...
def create_qualifying_properties(node, name=None, etsi='etsi'):
    obj_node = create_node('Object', node, DSigNs,  tail='\n')
...
def create_signed_properties(node, name=None, datetime=None):
    properties = create_node('SignedProperties', node, EtsiNS,  tail='\n')
    if name is not None:
        properties.set('Id', name)
    signature_properties = create_node(
        'SignedSignatureProperties', properties, EtsiNS,   tail='\n'
    )
    signing_time = create_node('SigningTime', signature_properties, EtsiNS,  tail='\n')
    if datetime is not None:
        signing_time.text = datetime.isoformat()
    create_node('SigningCertificate', signature_properties, EtsiNS,tail='\n')
    create_node('SignaturePolicyIdentifier', signature_properties, EtsiNS,tail='\n')
    return properties
